...
- [ ] T6 — BackupJson: Gson round-trip of blocked apps, schedules, settings (full spec: contracts/estate-build.md T6)
  - teacher-guidance: BackupJson: Implement Gson round-trip serialization for BlockedApp, Schedule, and Settings objects with strict rejection of malformed JSON (partial fields, version mismatches) in `app/src/main/java/com/piercingxx/nopemode/data/BackupJson.kt` and corresponding tests in `app/src/test/java/com/piercingxx/nopemode/data/BackupJsonTest.kt`, then execute `env JAVA_HOME=/home/piercingxx/.local/android-toolchain/jdk17 ANDROID_HOME=/home/piercingxx/.local/android-toolchain/android-sdk ./gradlew --no-daemon testDebugUnitTest --tests 'com.piercingxx.nopemode.data.BackupJsonTest'` to verify and flip the task status.
